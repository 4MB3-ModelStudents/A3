...
R0 <- seq(1,20,length=100)
eps <- seq(0.00001,0.999,length=100)

#Defining functions thats calculates the period
period <- function(eps, R0) {
  (4*pi)/sqrt(4*(R0-1)-(R0*eps)^2)
}
...
#Showing epsilon^*
epsstar <- 4*(R0-1)/R0^2 
lines (trans3d(epsstar, R0, rep(0,100), pmat = res), col = 4)
mtext("$\\epsilon^*(\\R_0)$", side=4, line=-8, padj=17, las=2, col=4)
...
R0 <- seq(1,20,length=100)
eps <- seq(0.0001,0.999,length=100)
