...
Forward Invarianace: A solution that tends to one equilibria $x_*$ in forward time. (i.e. $\forall x(t)\in\Delta\subseteq S$, $\lim_{t \to \infty} x(t) = x_* $ for $x_*\in\Delta$)  
}
\end{proof}
}
...
The equilibrium $(S^*, I^*)$ is asymptotically stable if all of the eigenvalues of the linearized system at $(S^*, I^*)$ have negative real parts. REFERENCE GOES HERE
...
\end{align}
Thus, L is a strict Lyapunov function and $\mathcal{C}$ is globally asymptotically stable if $\R_0 \leq 1$. All initial states in $\mathcal{B} \equiv \Delta \setminus \{(S,I): S = \frac{1}{\R_0}  \}$ (an open) are attracted to the DFE when $\R_0 \leq 1$. $\mathcal{B}$ is the basin of attraction of the DFE. It is an open, dense suset of $\Delta$. 
...
Since
\begin{equation}
  \frac{\partial^2 L}{\partial S^2} \frac{\partial^2 L}{\partial I^2} - \frac{\partial^2 L}{\partial I \partial S} = \left (1 + \frac{\Shat}{S^2} \right )\left (1 + \frac{\Ihat}{I^2} \right ) >0
\end{equation}
$(\Shat,\Ihat)$ is a minimum or maximum point and not a saddle point. Since
\begin{equation}
  \frac{\partial^2 L}{\partial S^2}>0, \quad \frac{\partial^2 L}{\partial I^2}>0 
\end{equation}
$(\Shat,\Ihat)$ is a minimum of $L(S,I)$ and thus
\begin{equation}
L(S,I)>L(\Shat,\Ihat) \quad \text{for all} \quad (S,I)\in\openset\setminus\{(\Shat,\Ihat)\}.
\end{equation}
where $\openset= \Delta \setminus \{ (S,I): S,I \neq 0 \}$.
\begin{align}
...
\begin{proof}
{\color{magenta}
The approach to the EE occurs via damped oscillations if and only if the linearized system about the EE has complex conjugate eigenvalues with negative real parts. This is because the general solution of the linearized system, with eigenvalues $\lambda_\pm$, is
\begin{equation}
\begin{cases}
  S(\tau) = a_+ \exp{\lambda_+ \tau} + a_- \exp{\lambda_- \tau} \\
  I(\tau) = b_+ \exp{\lambda_+ \tau} + b_- \exp{\lambda_- \tau}
\end{cases}
\end{equation}
If $\lambda_\pm = \alpha \pm i\beta$, the solutions can be rewritten as
\begin{equation}
\begin{cases}
