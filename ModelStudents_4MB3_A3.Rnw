...
<<Qiper,fig.height=6,dev="tikz">>=
R0 <- seq(1.01,70,length=1000)
esp <- seq(0.001,5,length=1000)
espstar <- function(R0){4*(R0-1)/R0^2}
period <- function(R0, esp) {
  if (esp < espstar(R0)){
    (4/pi)*sqrt(4*(R0-1)-(R0*esp)^2)
  } else{
    0
  }
}
#periodvec <- outer(R0,esp,period)
#persp(R0, esp, periodvec, theta = 30, phi = 30, expand = 0.5, col = "lightblue",
#       ltheta = 120, shade = 0.75, ticktype = "detailed",
#       xlab = "$\\R_0$", ylab = "$\\epsilon$", zlab = "Period")
#title(main = "Period of Damped Oscillations")
@ 

<<Qief,fig.height=6,dev="tikz">>=
R0 <- seq(1.01,70,length=1000)
esp <- seq(0.001,5,length=1000)
espstar <- function(R0){4*(R0-1)/R0^2}
efold <- function(R0, esp) {
  if (esp < espstar(R0)){
    2/(R0*esp)
  } else{
    0
...
#efoldvec <- outer(R0,esp,efold)
#persp(R0, esp, efoldvec, theta = 30, phi = 30, expand = 0.5, col = "lightblue",
#       ltheta = 120, shade = 0.75, ticktype = "detailed",
#       xlab = "$\\R_0$", ylab = "$\\epsilon$", zlab = "$e$-Folding Time")
